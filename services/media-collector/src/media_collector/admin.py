...
def start_artist_import(request: AdminArtistImportRequest, db: Session = Depends(get_db)) -> dict:
    ...
    payload["official_source_urls"] = source_urls[:20]
...
def query_recommendations(source: Source, refresh: bool = False, db: Session = Depends(get_db)) -> dict:
    return {"source": source.value} | trending_idol_recommendations(db, force_refresh=refresh)
...
def run_full_automation(request: AdminCollectionRequest, db: Session = Depends(get_db)) -> dict:
    ...
    return {"status": "accepted", "message": f"n8n {workflow_name} 전체 자동화를 시작했습니다. 수집 후 저위험 AI 초안은 정책 승인되어 예약 발행됩니다."}
...
def ai_drafts(draft_status: str, start_date: date | None = None, end_date: date | None = None, source: Source | None = None, db: Session = Depends(get_db)) -> list[dict]:
    ...
    source_query = f"&source={source.value}" if source else ""
    drafts = ai_worker_request("GET", f"/v1/drafts?status={draft_status}&limit=1000{source_query}")
    start_at, end_at = date_bounds(start_date, end_date)
...
def collection_window(request: AdminCollectionRequest) -> tuple[datetime | None, datetime | None]:
    ...
    if request.published_from and request.published_to:
        ...
        return published_after.astimezone(timezone.utc), published_before.astimezone(timezone.utc)
    if request.published_within_hours is not None:
        return datetime.now(timezone.utc) - timedelta(hours=request.published_within_hours), None
    return None, None
...
<div id="artist-pane-divider" class="pane-divider" tabindex="0" role="separator" aria-orientation="vertical" aria-label="아티스트 수집 영역 너비 조절"></div><section class="panel jobs-panel"><div class="pane-tabbar"><span class="pane-tab">아티스트 탐색기</span><span class="pane-tab-meta">COLLECTED</span></div><div class="jobs-head"><div><span class="section-eyebrow">ARTIST MANAGEMENT</span><h3>수집된 아티스트</h3><p class="panel-lead">수집 현황을 확인하고 아티스트별 자동 갱신 주기를 관리합니다.</p></div><button id="refresh-jobs" type="button">목록 새로고침</button></div><div class="jobs-toolbar"><div class="toolbar-heading"><div><strong>아티스트 목록</strong><span>이름과 수집 상태로 원하는 항목을 빠르게 찾으세요.</span></div><span id="toolbar-result-count">0개 표시</span></div><label class="search-field"><span>아티스트 검색</span><input id="job-search" type="search" placeholder="이름으로 검색" aria-label="아티스트 이름 검색"></label><label><span>수집 상태</span><select id="job-status" aria-label="수집 상태"><option value="all">모든 상태</option><option value="completed">완료</option><option value="running">갱신 중</option><option value="pending">갱신 대기</option><option value="failed">갱신 실패</option></select></label><label><span>목록 정렬</span><select id="job-sort" aria-label="정렬"><option value="updated-desc">최근 갱신순</option><option value="updated-asc">오래된 갱신순</option><option value="name">이름순</option></select></label></div><div class="jobs-meta"><span id="job-count">0개</span><span>목록은 5초마다 확인</span></div><div id="jobs" class="jobs"><p class="empty">완료된 아티스트를 불러오는 중입니다.</p></div></section><section class="activity-log" aria-labelledby="activity-log-title"><div class="activity-log-head"><div><h3 id="activity-log-title">수집 진행 로그</h3><p>공식 출처 탐색과 데이터 반영 과정을 실시간으로 확인합니다.</p></div><select id="log-job" aria-label="로그를 확인할 수집 작업"></select><button id="refresh-log" type="button">로그 새로고침</button></div><div id="activity-log-body" class="activity-log-body" role="log" aria-live="polite"><p class="log-empty">수집 실행 후 진행 로그가 여기에 표시됩니다.</p></div></section></form></div></main>
<script>
...
.field-label-actions{display:flex;align-items:center;justify-content:space-between;gap:14px}.field-label-actions label{margin:0}.field-label-actions button{min-height:38px;padding:8px 13px;border:1px solid #816cf0;background:#292143;color:#ded7ff;font-size:13px;white-space:nowrap}.field-label-actions button:hover{border-color:#ad9dff;background:#352957}.field-label-actions button:disabled{opacity:.6}.source-field-help{display:flex;align-items:flex-start;justify-content:space-between;gap:18px}.source-field-help small:last-child{color:#aaa0db;text-align:right}.source-field-help small.success{color:var(--ok)}.source-field-help small.error{color:#ff91a8}
body.artist-list-page{background:#0b0e13}
body.artist-list-page .content{background:radial-gradient(circle at 85% 0,#22203b55,transparent 32%)}
body.artist-list-page .layout>.jobs-panel{max-width:1480px;padding:24px 40px 56px!important}
body.artist-list-page .pane-tabbar{display:none}
body.artist-list-page .jobs-head{display:flex;align-items:center;justify-content:space-between;gap:28px;padding:0 0 24px}
body.artist-list-page .jobs-head>div:first-child{min-width:0}
body.artist-list-page .jobs-head h3{margin:4px 0 4px;font-size:28px;letter-spacing:-.02em}
body.artist-list-page .jobs-head .panel-lead{margin:0;color:#98a1b0;font-size:14px}
body.artist-list-page .section-eyebrow{color:#9d88ff;font-size:12px;font-weight:900;letter-spacing:.12em}
body.artist-list-page .jobs-head-actions{display:flex;align-items:center;gap:10px;flex:0 0 auto}
body.artist-list-page .artist-page-nav{margin:0;order:2}
body.artist-list-page .artist-page-nav a,body.artist-list-page #refresh-jobs{display:inline-flex;align-items:center;justify-content:center;min-height:44px;padding:10px 16px;border-radius:9px;font-size:14px;font-weight:800;text-decoration:none}
body.artist-list-page .artist-page-nav a{border:1px solid #9177ff;background:linear-gradient(135deg,#7859e9,#9b5de5);color:#fff;box-shadow:0 8px 24px #7658e833}
body.artist-list-page .artist-page-nav a:hover{border-color:#b4a3ff;background:linear-gradient(135deg,#886bf0,#aa6bed)}
body.artist-list-page #refresh-jobs{margin:0;border:1px solid #465061;background:#202630;color:#e9ecf1}
body.artist-list-page #refresh-jobs:hover{border-color:#68758a;background:#29313e}
body.artist-list-page .artist-overview{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin:0 0 20px}
body.artist-list-page .artist-overview article{position:relative;min-height:138px;overflow:hidden;padding:18px 20px;border:1px solid #303846;border-radius:14px;background:linear-gradient(145deg,#191e27,#12161d);box-shadow:0 12px 30px #0003}
body.artist-list-page .artist-overview article::after{content:"";position:absolute;right:-24px;bottom:-38px;width:110px;height:110px;border-radius:50%;background:var(--metric-color,#9d88ff);opacity:.07}
body.artist-list-page .metric-head{display:flex;align-items:center;gap:10px;color:#b7bfcb;font-size:13px;font-weight:800}
body.artist-list-page .metric-icon{display:grid;place-items:center;width:34px;height:34px;border:1px solid color-mix(in srgb,var(--metric-color,#9d88ff) 45%,#303846);border-radius:9px;background:color-mix(in srgb,var(--metric-color,#9d88ff) 13%,transparent);color:var(--metric-color,#9d88ff);font-size:14px;font-weight:900}
body.artist-list-page .metric-value{display:flex;align-items:baseline;gap:5px;margin:13px 0 5px}
body.artist-list-page .metric-value strong{color:var(--metric-color,#d9d2ff);font-size:34px;line-height:1}
body.artist-list-page .metric-value small{color:#858f9f;font-size:13px;font-weight:700}
body.artist-list-page .artist-overview em{display:block;color:#858f9f;font-size:12px;font-style:normal}
body.artist-list-page .artist-overview article[data-tone="active"]{--metric-color:#62d49c}
body.artist-list-page .artist-overview article[data-tone="pending"]{--metric-color:#ffd166}
body.artist-list-page .artist-overview article[data-tone="failed"]{--metric-color:#ff718d}
body.artist-list-page .jobs-toolbar{display:grid;grid-template-columns:minmax(280px,1fr) 190px 190px;gap:14px;margin:0;padding:0 0 20px;border-bottom:1px solid #2d3440;background:transparent}
body.artist-list-page .toolbar-heading{grid-column:1/-1;display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:2px}
body.artist-list-page .toolbar-heading div{min-width:0}
body.artist-list-page .toolbar-heading strong{display:block;color:#f0f2f5;font-size:16px}
body.artist-list-page .toolbar-heading div span{display:block;margin-top:3px;color:#858f9f;font-size:13px}
body.artist-list-page #toolbar-result-count{padding:5px 10px;border:1px solid #3c4554;border-radius:999px;background:#171c24;color:#c8ced8;font-size:12px;font-weight:800;white-space:nowrap}
body.artist-list-page .jobs-toolbar label{display:flex;min-width:0;flex-direction:column;gap:7px;color:#aeb6c4;font-size:12px;font-weight:800}
body.artist-list-page .jobs-toolbar input,body.artist-list-page .jobs-toolbar select{width:100%;min-height:46px;padding:10px 13px;border:1px solid #3b4555;border-radius:9px;background:#10141a;color:#f5f6f8;font-size:14px;box-shadow:inset 0 1px 0 #ffffff05}
body.artist-list-page .jobs-toolbar .search-field input{padding-left:15px;background:linear-gradient(90deg,#17152a,#10141a 80%)}
body.artist-list-page .jobs-toolbar input:hover,body.artist-list-page .jobs-toolbar select:hover{border-color:#566276}
body.artist-list-page .jobs-toolbar input:focus,body.artist-list-page .jobs-toolbar select:focus{border-color:#9a84ff;outline:3px solid #8b6cff26}
body.artist-list-page .jobs-meta{display:flex;justify-content:space-between;margin:13px 2px;color:#949dac;font-size:13px}
body.artist-list-page .jobs-meta #job-count{color:#e4e7ec;font-weight:800}
body.artist-list-page .jobs{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;max-height:none;margin:0!important;padding:0!important;overflow:visible}
body.artist-list-page .job{position:relative;display:grid!important;grid-template-columns:52px minmax(0,1fr);align-content:start;gap:14px 13px;min-width:0;padding:18px!important;border:1px solid #303746!important;border-radius:14px!important;background:linear-gradient(145deg,#181c24,#12151b)!important;box-shadow:0 12px 30px #0003;transition:border-color .15s,transform .15s,box-shadow .15s}
body.artist-list-page .job:hover{border-color:#7568a8!important;transform:translateY(-2px);box-shadow:0 16px 38px #0005}
body.artist-list-page .artist-card-avatar{display:grid;grid-column:1;grid-row:1;place-items:center;width:52px;height:52px;border:1px solid #554a7d;border-radius:13px;background:linear-gradient(145deg,#3b2d64,#251e3d);color:#d9d1ff;font-size:20px;font-weight:900}
body.artist-list-page .job-top{grid-column:2;grid-row:1;min-width:0}
body.artist-list-page .job-top>div{min-width:0}
body.artist-list-page .artist-detail-link{display:block;overflow:hidden;color:#f4f5f8;font-size:17px;font-weight:850;text-decoration:none;text-overflow:ellipsis;white-space:nowrap}
body.artist-list-page .artist-detail-link:hover{color:#c5b8ff;text-decoration:underline}
body.artist-list-page .job-top time{display:block;margin-top:4px;color:#9099a8;font-size:12px}
body.artist-list-page .state{flex:0 0 auto;margin-left:10px;padding:5px 8px;border-radius:999px;background:#11151b;font-size:12px;font-weight:800}
body.artist-list-page .job-stage{grid-column:1/-1;padding:12px;border:1px solid #2d3440;border-radius:9px;background:#0e1117}
body.artist-list-page .job-stage b{font-size:13px}
body.artist-list-page .job-stage small{font-size:12px}
body.artist-list-page .job-refresh{grid-column:1/-1;margin-top:0;padding-top:14px}
body.artist-list-page .job-refresh label{font-size:12px}
body.artist-list-page .job-refresh select,body.artist-list-page .job-refresh button{min-height:42px;font-size:13px}
body.artist-list-page .artist-card-action{grid-column:1/-1;display:flex;align-items:center;justify-content:center;min-height:40px;border:1px solid #4b426b;border-radius:8px;background:#201b31;color:#c8bbff;font-size:13px;font-weight:800;text-decoration:none}
body.artist-list-page .artist-card-action:hover{border-color:#927cff;background:#29213f}
body.artist-list-page .empty{grid-column:1/-1;padding:40px;border:1px dashed #3f4858;border-radius:12px;background:#12161d;text-align:center}
body.artist-detail-page .left-pane>.pane-tabbar{position:sticky;top:0;z-index:8;margin:0 0 24px;border-bottom:1px solid #303746;background:#0e1015}
body.artist-detail-page .detail-tab{position:relative;min-height:48px;padding:0 20px;border:0;border-right:1px solid #2f3541;border-radius:0;background:transparent;color:#929cac;font-size:14px;font-weight:800}
body.artist-detail-page .detail-tab[aria-selected="true"]{background:#181b22;color:#f5f6f8}
body.artist-detail-page .detail-tab[aria-selected="true"]::after{content:"";position:absolute;right:0;bottom:0;left:0;height:3px;background:var(--hot)}
body.artist-detail-page .detail-tab-count{margin-left:7px;padding:2px 7px;border-radius:999px;background:#2d3441;color:#bfc7d4;font-size:12px}
body.artist-detail-page .detail-tab[aria-selected="true"] .detail-tab-count{background:#493143;color:#ffb1c0}
body.artist-detail-page .result-pane{padding:0 0 28px}
body.artist-detail-page .result-hero{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:18px}
body.artist-detail-page .result-hero h2{margin:4px 0 6px;font-size:26px}
body.artist-detail-page .result-hero p{margin:0;color:#98a1b0;font-size:14px}
body.artist-detail-page .result-state{padding:7px 11px;border:1px solid #465061;border-radius:999px;background:#171c24;color:#cbd2dd;font-size:13px;font-weight:800;white-space:nowrap}
body.artist-detail-page .result-state.completed{border-color:#357356;color:#72dda8}body.artist-detail-page .result-state.partial,body.artist-detail-page .result-state.needs_attention{border-color:#7d642d;color:#ffd166}body.artist-detail-page .result-state.failed{border-color:#783746;color:#ff91a8}
body.artist-detail-page .result-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:18px}
body.artist-detail-page .result-summary article{padding:16px;border:1px solid #303746;border-radius:12px;background:#151921}
body.artist-detail-page .result-summary span{display:block;color:#919bab;font-size:12px;font-weight:700}
body.artist-detail-page .result-summary strong{display:block;margin-top:8px;color:#f4f6f8;font-size:22px}
body.artist-detail-page .result-section{margin-top:18px;padding-top:18px;border-top:1px solid #303746}
body.artist-detail-page .result-section-head{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:12px}
body.artist-detail-page .result-section h3{margin:0;font-size:20px}body.artist-detail-page .result-section-head span{color:#9099a8;font-size:13px}
body.artist-detail-page .result-scope-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}
body.artist-detail-page .result-scope-card{min-height:112px;padding:14px;border:1px solid #303746;border-radius:11px;background:#141820}
body.artist-detail-page .result-scope-card header{display:flex;align-items:center;justify-content:space-between;gap:8px}
body.artist-detail-page .result-scope-card h4{margin:0;font-size:14px}.result-scope-card .scope-state{padding:3px 7px;border-radius:999px;background:#252c37;color:#aeb7c5;font-size:12px;font-weight:800}.result-scope-card .scope-state.complete{background:#173729;color:#72dda8}.result-scope-card .scope-state.warning{background:#3b311b;color:#ffd166}.result-scope-card .scope-state.skipped{background:#242933;color:#87919f}
body.artist-detail-page .result-scope-card strong{display:block;margin-top:13px;font-size:20px}.result-scope-card p{margin:4px 0 0;color:#8e98a7;font-size:12px;line-height:1.5}
body.artist-detail-page .result-content-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
body.artist-detail-page .result-content-card{min-width:0;padding:16px;border:1px solid #303746;border-radius:12px;background:#141820}
body.artist-detail-page .result-content-card.wide{grid-column:1/-1}.result-content-card h4{margin:0 0 10px;font-size:16px}.result-content-card p{margin:7px 0;color:#c8ced8;font-size:13px;line-height:1.65;overflow-wrap:anywhere}.result-content-card small{color:#8d97a7;font-size:12px}.result-content-card a{color:#b9a8ff;overflow-wrap:anywhere}.result-content-card ul{margin:8px 0 0;padding-left:19px}.result-content-card li{margin:6px 0;color:#c8ced8;font-size:13px;line-height:1.55}.result-content-card details{margin-top:10px;border-top:1px solid #2e3541}.result-content-card summary{padding:10px 0;color:#c8baff;font-size:13px;font-weight:800;cursor:pointer}
body.artist-detail-page .result-empty{padding:34px;border:1px dashed #414b5c;border-radius:12px;background:#12161d;color:#98a1b0;font-size:14px;text-align:center}
body.artist-detail-page .result-issues{display:grid;gap:9px}.result-issue{padding:14px;border:1px solid #6f5930;border-radius:10px;background:#211c13}.result-issue strong{display:block;color:#ffd166;font-size:14px}.result-issue p{margin:6px 0 0;color:#d3c7ac;font-size:13px;line-height:1.6}
@media(max-width:1100px){body.artist-list-page .jobs{grid-template-columns:repeat(2,minmax(0,1fr))}body.artist-list-page .artist-overview,body.artist-detail-page .result-summary,body.artist-detail-page .result-scope-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:700px){.hero-run-wrap{grid-template-columns:1fr}.collection-timing{min-width:0}body .jobs-toolbar{grid-template-columns:1fr}body .layout>.left-pane,body .layout>.jobs-panel{padding:16px!important}body.artist-list-page .layout>.jobs-panel{padding:20px 16px 40px!important}body.artist-list-page .jobs-head{align-items:stretch;flex-direction:column;gap:18px}body.artist-list-page .jobs-head-actions{width:100%}body.artist-list-page .artist-page-nav,body.artist-list-page .artist-page-nav a,body.artist-list-page #refresh-jobs{flex:1;width:100%}body.artist-list-page .artist-overview{grid-template-columns:repeat(2,minmax(0,1fr))}body.artist-list-page .artist-overview article{min-height:126px;padding:16px}body.artist-list-page .jobs{grid-template-columns:1fr}body.artist-list-page .jobs-toolbar{padding:0 0 18px}body.artist-list-page .toolbar-heading{align-items:flex-start;flex-direction:column}body.artist-list-page .jobs-meta{align-items:flex-start;flex-direction:column;gap:4px}}
@media(max-width:700px){.source-field-help{flex-direction:column;gap:5px}.source-field-help small:last-child{text-align:left}}
@media(max-width:460px){body.artist-list-page .artist-overview{grid-template-columns:1fr}body.artist-list-page .jobs-head-actions{flex-direction:column}body.artist-list-page .metric-value strong{font-size:30px}body.artist-detail-page .result-summary,body.artist-detail-page .result-scope-grid,body.artist-detail-page .result-content-grid{grid-template-columns:1fr}body.artist-detail-page .result-hero{flex-direction:column}body.artist-detail-page .detail-tab{padding:0 13px}}
`;document.head.appendChild(pageStyle);
const originalRenderJobs=renderJobs;
renderJobs=function(){
 originalRenderJobs();
 $('toolbar-result-count').textContent=$('job-count').textContent+' 표시';
 document.querySelectorAll('#jobs .job').forEach(card=>{
  const id=card.dataset.artistId,job=artistJobs.find(item=>String(item.artist_id||'')===id);
  if(!job)return;
  card.dataset.status=job.status;
  const href='/admin/artists?detail='+encodeURIComponent(job.job_id),title=card.querySelector('.job-top strong'),link=document.createElement('a');
  link.className='artist-detail-link';link.href=href;link.textContent=job.artist_name;link.setAttribute('aria-label',job.artist_name+' 수집 설정 열기');title.replaceWith(link);
  const avatar=document.createElement('span');avatar.className='artist-card-avatar';avatar.setAttribute('aria-hidden','true');avatar.textContent=String(job.artist_name||'?').trim().charAt(0).toLocaleUpperCase('ko-KR')||'?';card.prepend(avatar);
  const action=document.createElement('a');action.className='artist-card-action';action.href=href;action.textContent='수집 설정 및 상세 보기';card.append(action);
 });
...
if(isDetail)document.querySelector('.left-pane').prepend(nav);
else{const actions=document.createElement('div'),head=document.querySelector('.jobs-head'),refresh=$('refresh-jobs');actions.className='jobs-head-actions';actions.append(refresh,nav);head.append(actions)}
const artistOverview=document.createElement('section');
artistOverview.className='artist-overview';artistOverview.setAttribute('aria-label','아티스트 수집 현황');
if(!isDetail)document.querySelector('.jobs-toolbar').before(artistOverview);
function renderArtistOverview(){
 if(isDetail)return;
 const counts={completed:0,active:0,pending:0,failed:0};
 artistJobs.forEach(job=>{if(job.status==='completed')counts.completed+=1;if(job.status==='running')counts.active+=1;if(job.status==='pending'||job.status==='needs_attention')counts.pending+=1;if(job.status==='failed'||job.status==='cancelled')counts.failed+=1});
 artistOverview.innerHTML=`<article><div class="metric-head"><span class="metric-icon" aria-hidden="true">ALL</span><span>전체 아티스트</span></div><p class="metric-value"><strong>${artistJobs.length}</strong><small>명</small></p><em>Collector에 등록된 전체 대상</em></article><article data-tone="active"><div class="metric-head"><span class="metric-icon" aria-hidden="true">OK</span><span>정상 수집 완료</span></div><p class="metric-value"><strong>${counts.completed}</strong><small>명</small></p><em>최근 작업이 정상 완료된 대상</em></article><article data-tone="pending"><div class="metric-head"><span class="metric-icon" aria-hidden="true">···</span><span>진행 및 확인 대기</span></div><p class="metric-value"><strong>${counts.active+counts.pending}</strong><small>명</small></p><em>처리 중이거나 확인이 필요한 대상</em></article><article data-tone="failed"><div class="metric-head"><span class="metric-icon" aria-hidden="true">!</span><span>실패 및 중지</span></div><p class="metric-value"><strong>${counts.failed}</strong><small>명</small></p><em>재실행 또는 점검이 필요한 대상</em></article>`;
}
const resultPane=document.createElement('section'),detailTabbar=document.querySelector('.left-pane>.pane-tabbar'),activityLog=document.querySelector('.activity-log');
resultPane.id='artist-result-pane';resultPane.className='result-pane';resultPane.hidden=true;document.querySelector('.left-pane').append(resultPane);
if(isDetail){detailTabbar.setAttribute('role','tablist');detailTabbar.setAttribute('aria-label','아티스트 수집 상세');detailTabbar.innerHTML='<button class="detail-tab" type="button" role="tab" aria-selected="true" aria-controls="artist-editor-pane" data-detail-tab="editor">수집 편집기</button><button class="detail-tab" type="button" role="tab" aria-selected="false" aria-controls="artist-result-pane" data-detail-tab="result" tabindex="-1">수집 결과 <span id="result-tab-count" class="detail-tab-count">대기</span></button><span class="pane-tab-meta">ARTIST IMPORT</span>';document.querySelector('.left-pane>.hero').id='artist-editor-pane';activityLog.hidden=true}
const editorNodes=[...document.querySelectorAll('.left-pane>.hero,.left-pane>.settings-panel,.left-pane>.scope-panel')];
function activateArtistDetailTab(name){if(!isDetail)return;const result=name==='result';editorNodes.forEach(node=>node.hidden=result);resultPane.hidden=!result;activityLog.hidden=!result;detailTabbar.querySelectorAll('[data-detail-tab]').forEach(button=>{const active=button.dataset.detailTab===name;button.setAttribute('aria-selected',String(active));button.tabIndex=active?0:-1})}
if(isDetail){detailTabbar.addEventListener('click',event=>{const button=event.target.closest('[data-detail-tab]');if(button)activateArtistDetailTab(button.dataset.detailTab)});detailTabbar.addEventListener('keydown',event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;const tabs=[...detailTabbar.querySelectorAll('[data-detail-tab]')],current=tabs.indexOf(document.activeElement),next=event.key==='Home'?0:event.key==='End'?tabs.length-1:(current+(event.key==='ArrowRight'?1:-1)+tabs.length)%tabs.length;event.preventDefault();activateArtistDetailTab(tabs[next].dataset.detailTab);tabs[next].focus()})}
const resultLink=(url,label)=>{const value=String(url||''),safe=value.toLocaleLowerCase().startsWith('https://')||value.toLocaleLowerCase().startsWith('http://');return safe?`<a href="${esc(value)}" target="_blank" rel="noopener noreferrer">${esc(label||value)}</a>`:esc(label||value||'—')};
const resultEntryText=entry=>{if(typeof entry==='string')return entry;if(!entry||typeof entry!=='object')return '';return entry.text||[entry.year,entry.date,entry.title,entry.description,entry.detail].filter(Boolean).join(' · ')||JSON.stringify(entry)};
function renderArtistResult(job){
 if(!isDetail)return;
 ...
 resultPane.innerHTML=`<div class="result-hero"><div><div class="eyebrow">COLLECTION RESULT</div><h2>${esc(job.artist_name)} · 수집 결과</h2><p>마지막 확인 ${esc(itemTime(r.checked_at||job.completed_at||job.updated_at))}</p></div><span class="result-state ${quality==='partial'?'partial':esc(status)}">${quality==='complete'?'수집 완료':quality==='partial'?'일부 수집 · 보완 필요':esc(stateLabels[status]||status)}</span></div><section class="result-summary" aria-label="수집 결과 요약"><article><span>작업 진행률</span><strong>${Number(job.progress)||0}%</strong></article><article><span>반영된 항목</span><strong>${Number(job.collected)||0}개</strong></article><article><span>확인한 출처</span><strong>${Number(r.sources_checked)||0}개</strong></article><article><span>공개 상태</span><strong>${publication}</strong></article></section><section class="result-section"><div class="result-section-head"><h3>항목별 수집 현황</h3><span>선택한 수집 범위 기준</span></div><div class="result-scope-grid">${scopeCards}</div></section><section class="result-section"><div class="result-section-head"><h3>수집된 내용과 근거</h3><span>공식 출처 기반 결과</span></div><div class="result-content-grid"><article class="result-content-card"><h4>프로필</h4><p><b>공식 데뷔일:</b> ${esc(profile.debut_date||'미확인')}</p>${profile.source_url?`<p>${resultLink(profile.source_url,'프로필 공식 근거')}</p>`:''}${profileParagraphs||'<p>확인된 프로필 본문이 없습니다.</p>'}</article><article class="result-content-card"><h4>공식 SNS</h4>${socialItems?`<ul>${socialItems}</ul>`:'<p>소유 근거가 확인된 공식 SNS가 없습니다.</p>'}</article>${writingCards}<article class="result-content-card wide"><h4>앨범과 수록곡</h4>${albumItems||'<p>확인된 앨범과 수록곡이 없습니다.</p>'}</article><article class="result-content-card wide"><h4>갤러리 판별</h4><p>활동 사진 후보 ${galleryCounts[0]}개 · 확인 필요 ${galleryCounts[1]}개 · 제외 ${galleryCounts[2]}개 · 미검토 ${Number(gallery.remaining)||0}개</p>${galleryLinks?`<ul>${galleryLinks}</ul>`:'<p>기록된 이미지 판별 항목이 없습니다.</p>'}</article><article class="result-content-card wide"><h4>입력 및 확인 출처</h4><ul>${(settings.official_source_urls||[]).map(url=>`<li>${resultLink(url,url)}</li>`).join('')||'<li>직접 입력한 공식 출처가 없습니다.</li>'}</ul></article></div></section>${issues.length?`<section class="result-section"><div class="result-section-head"><h3>보완이 필요한 항목</h3><span>${issues.length}건</span></div><div class="result-issues">${issueCards}</div></section>`:''}`;
}
...
<body><main>
  ...
  <div class="workspace">
    <aside class="panel setup-panel"><div id="pipeline-setup-cover" class="pipeline-cover" hidden><div class="pipeline-cover-card"><i class="pipeline-spinner" aria-hidden="true"></i><strong>AI 초안 생성 중</strong><span id="pipeline-setup-text">선택한 미디어를 분석하고 있습니다.</span></div></div><div class="eyebrow">SETTINGS</div><h2>작업 설정</h2>
      ...
      </div></section><p class="automation-note"><strong>수집만 실행:</strong> 미디어 수집만 진행하며 AI 초안이나 게시물을 만들지 않습니다.<br><strong>전체 자동화:</strong> 수집 → AI 초안 생성 → 저위험 초안 정책 승인 → 예약 발행</p><div class="actions"><button id="run" type="submit">수집만 실행</button><button id="run-all" type="button" class="secondary">전체 자동화 실행</button></div></form>
...
function renderNewsRecommendations(){const list=$('news-idol-list'),artists=newsRecommendations.artists||[],naverOk=newsRecommendations.naver_trend?.status==='ok';list.innerHTML=artists.length?artists.map(artist=>{const trend=artist.naver_trend_score==null?'네이버 자료 없음':`네이버 ${Number(artist.naver_trend_score).toFixed(1)}`,origin=artist.discovered_from==='naver_news'?'네이버 뉴스 발견':'FANHEAT 아티스트';return `<span class="news-idol-chip" title="${esc(origin)} · ${esc(trend)} · 최근 언급 ${Number(artist.recent_mentions)||0}건 · ${esc((artist.activity_keywords||[]).join(', '))}">${esc(artist.name)} · ${esc(trend)}</span>`}).join(''):`<span class="news-recommendation-empty">최근 K-POP 뉴스에서 추천 키워드를 찾지 못했습니다.</span>`;if(!naverOk&&artists.length)list.insertAdjacentHTML('beforeend','<span class="news-recommendation-empty">네이버 트렌드 조회 실패 · 뉴스 언급 순위 사용</span>')}
async function loadNewsRecommendations(forceRefresh=false){const applyButton=$('apply-news-recommendations'),refreshButton=$('refresh-news-recommendations');applyButton.disabled=true;refreshButton.disabled=true;if(forceRefresh){refreshButton.textContent='찾는 중…';$('news-idol-list').innerHTML='<span class="news-recommendation-empty">네이버 최신 검색 트렌드를 다시 확인하고 있습니다.</span>'}try{newsRecommendations=await api(`/admin/api/query-recommendations/${currentSource()}${forceRefresh?'?refresh=true':''}`);renderNewsRecommendations();if(forceRefresh){$('status').className='status done';$('status').textContent=(newsRecommendations.artists||[]).length?`최신 추천 아티스트 ${(newsRecommendations.artists||[]).length}명을 찾았습니다.`:'추천 아티스트를 찾지 못했습니다. 공개 아티스트와 네이버 API 설정을 확인하세요.'}}catch(error){$('news-idol-list').innerHTML=`<span class="news-recommendation-empty">${esc(error.message)}</span>`;$('status').className='status failed';$('status').textContent=`추천 키워드 조회 실패 · ${error.message}`}finally{applyButton.disabled=false;refreshButton.disabled=false;refreshButton.textContent='다시 찾아보기'}}
$('refresh-news-recommendations').addEventListener('click',()=>loadNewsRecommendations(true));
...
function effectiveMediaStatus(row){return row.draft_status||(row.enrichment_status==='failed'?'failed':'unprocessed')}
function mediaMatchesFilter(row,filter){const status=effectiveMediaStatus(row);if(filter==='all')return true;if(filter==='review')return status==='generated'||status==='review';if(filter==='approved')return status==='approved'||status==='scheduled';if(filter==='rejected')return status==='rejected'||status==='failed';return status===filter}
function renderMediaCard(row){const status=effectiveMediaStatus(row),state=`<span class="media-state ${esc(status)}">${esc(mediaStatusLabels[status]||status)}</span>`,check=`<input class="media-check" type="checkbox" data-media-check="${esc(row.id)}" aria-label="${esc(row.title)} 선택">`,classes=`media-card ${row.draft_status?'locked':''}`;if(row.source==='x'){const author=row.author||{},handle=author.handle?`@${author.handle}`:'X 작성자';return `<article class="${classes} x-card">${check}${state}<a href="${esc(row.url)}" target="_blank" rel="noreferrer"><div class="x-post-content"><div class="x-post-head"><i class="x-post-logo">𝕏</i><div><b>${esc(author.name||handle)}</b><span>${esc(handle)}</span></div></div><p>${esc(row.title)}</p><small>${row.published_at?esc(new Date(row.published_at).toLocaleString()):''} · X에서 보기 ↗</small></div></a></article>`}return `<article class="${classes}">${check}${state}<a href="${esc(row.url)}" target="_blank" rel="noreferrer">${row.thumbnail_url?`<img src="${esc(row.thumbnail_url)}" alt="" loading="lazy">`:''}<div><strong>${esc(row.title)}</strong><small>${esc(sourceLabels[row.source]||row.source)} · ${row.published_at?esc(new Date(row.published_at).toLocaleString()):''}</small></div></a></article>`}
function renderMedia(){const filter=$('media-status-filter').value,source=currentSource();selectedMedia.clear();visibleMedia=loadedMedia.filter(row=>mediaMatchesFilter(row,filter));const count=visibleMedia.length,total=loadedMedia.length;$('media-result').textContent=filter==='all'?`${total}개`:`${count}/${total}개`;$('media').innerHTML=visibleMedia.map(renderMediaCard).join('')||`<p class="result">${filter==='all'?'선택한 기간에 수집된':'선택한 상태에 해당하는'} ${esc(sourceLabels[source])} 콘텐츠가 없습니다.</p>`;updateMediaSelection()}
async function loadMedia(){selectedMedia.clear();updateMediaSelection();try{const source=currentSource();loadedMedia=await loadAllPages(`/admin/api/media?source=${encodeURIComponent(source)}`);renderMedia()}catch(e){loadedMedia=[];visibleMedia=[];$('media-result').textContent=e.message}}
...
$('reload-media').addEventListener('click',loadMedia);$('media-status-filter').addEventListener('change',renderMedia);$('reload-drafts').addEventListener('click',loadDrafts);$('draft-status').addEventListener('change',loadDrafts);document.querySelectorAll('[data-draft-type]').forEach(button=>button.addEventListener('click',()=>setDraftType(button.dataset.draftType)));
...
$('publish-ai').addEventListener('click',async()=>{const ids=visibleDrafts.filter(draft=>(draft.status==='approved'||draft.status==='scheduled')&&draft.profile_id).map(draft=>String(draft.id));if(!ids.length||!confirm('현재 발행 가능한 승인 초안을 모두 FANHEAT 사용자 피드에 바로 게시할까요?'))return;await publishDrafts(ids,$('publish-ai'))});
