create table if not exists public.artist_click_events (
...
create index if not exists artist_click_events_period_idx
...
begin
  ...
  with metrics as (
    select
      ...
      (
        count(distinct followers.user_id)
        + count(distinct fans.profile_id)
      )::bigint as fan_growth,
      0::bigint as gallery_growth
    from public.artists a
    left join public.artist_click_events clicks
      on clicks.artist_id = a.id and clicks.clicked_at >= v_since
    left join public.artist_followers followers
      on followers.artist_id = a.id and followers.created_at >= v_since
    left join public.artist_fans fans
      on fans.artist_id = a.id and fans.active = true and fans.created_at >= v_since
    where a.active = true
    group by a.id
  ), scored as (
    select metrics.*, (metrics.click_count + metrics.fan_growth * 3)::bigint as ranking_score
    from metrics
  ), ranked as (
    select scored.*, rank() over (
      order by scored.ranking_score desc, scored.click_count desc,
        scored.fan_growth desc, scored.artist_id
    ) as rank
    from scored
  )
  select ranked.artist_id, ranked.click_count, ranked.fan_growth, ranked.gallery_growth,
         ranked.ranking_score, ranked.rank
  from ranked
  order by ranked.ranking_score desc, ranked.click_count desc, ranked.fan_growth desc, ranked.artist_id
  limit 50;
end;
$$;
