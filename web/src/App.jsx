...
import { addComment, castDailyArtistVote, deleteComment, deleteProfileGalleryImage, loadBestFriends, loadCommentReactions, loadComments, loadDailyArtistVotes, loadFanCred, loadHomeData, loadMessageBlocks, loadMessageContacts, loadPostBookmark, loadPostVote, loadPrivateMessages, loadProfileCustomization, loadProfileGallery, loadUnreadMessageCount, loadUserBookmarks, loadUserComments, markPrivateMessageRead, publishPost, saveProfileCustomization, sendPrivateMessage, setCommentReaction, setMessageBlock, setPostBookmark, setPostVote, submitFanPhotos, updateComment, updatePost, uploadProfileGalleryImages } from './lib/api'
import { calculateFanLevel, calculateFanRank, calculateHeatSnapshot } from './lib/fan-stats'
...
const makeStarProfile = ([name, score, image, data]) => {
  const galleryItems = [...(data?.artist_gallery_items || [])].sort((a, b) => Number(a.display_order || 0) - Number(b.display_order || 0))
  ...
  return {
    ...
    heroImage: data?.hero_image_url || image,
...
function ChartPanel({ onPlay, activeSong, songPlaying = false, items = charts, artists = [], collapsed, onToggle, user, onLogin, initialVoteMode = false, dedicatedVotePage = false }) {
  ...
  const openVoting = () => {
    ...
    if (ownVote) { setVoteMessage('오늘의 투표를 이미 완료했습니다.'); return }
  ...
  return <aside className={`chart-panel ${collapsed ? 'collapsed' : ''} ${voteMode ? 'voting-mode' : ''}`.trim()}>
    <div className="brand-tile">
      ...
      {dedicatedVotePage && !voteMode && <p className={`daily-vote-state ${ownVote ? 'complete' : ''}`}>{ownVote ? `오늘 투표 완료 · ${localizeTitle(ownVoteItem?.[0] || '선택한 아티스트')}` : '오늘 아직 투표하지 않았어요'}</p>}
...
function Hero({ user, onLogin }) {
  const slides = [
    ['bingle_bangle.jpg', 'bg_hyuna.jpg', 'AOA · Bingle Bangle', 'AOA 5TH MINI ALBUM · BINGLE BANGLE', 'AOA'],
    ['rescene-jacket.jpeg', 'rescene-bg.jpeg', 'RESCENE · Pretty Girl', '2026 SPECIAL SINGLE · PRETTY GIRL', 'RESCENE'],
    ['ive-jacket.jpeg', 'ive-bg.jpeg', 'IVE · REVIVE+', 'IVE THE 2ND ALBUM · BLACKHOLE', 'IVE'],
  ]
...
function SharedHeader({ query, setQuery, menuOpen, setMenuOpen, loggedIn, user, unreadMessageCount = 0, onLogin, onWrite, onHome, onMyPage, onLogout, writing, searchFilters, setSearchFilters, filterAuthors = [] }) {
...
function ProfileEditPanel({ initial, music, gallery, busy, status, onAddGallery, onRemoveGallery, onClose, onSave }) {
  const [values, setValues] = useState(() => ({
    ...
    bio: initial.bio || '',
    facebookUrl: initial.facebook_url || '',
    xUrl: initial.x_url || '',
    instagramUrl: initial.instagram_url || '',
    tiktokUrl: initial.tiktok_url || '',
  ...
  const mediaPicker = (items, setter, kind, label) => <div className={`profile-media-strip media-${kind}`}>
    {items.map((item, index) => <figure key={item.id}><img src={item.preview} alt={`${label} ${index + 1} 미리보기`} onError={event => { event.currentTarget.onerror = null; event.currentTarget.src = kind === 'avatar' ? '/images/mypage.jpg' : '/images/mypage_bg.jpg'; event.currentTarget.classList.add('is-fallback') }} /><button type="button" onClick={() => removeImage(item.id, setter)} aria-label={`${label} ${index + 1} 삭제`}>×</button></figure>)}
  ...
  return createPortal(<div className="profile-edit-workspace"><section className="profile-edit-panel" aria-labelledby="profile-edit-title"><header><div><small>MY PROFILE</small><h2 id="profile-edit-title">프로필 수정</h2><p>오른쪽 편집 영역에서 정보를 수정하면 왼쪽 프로필에 반영됩니다.</p></div><button type="button" onClick={onClose} aria-label="프로필 수정 종료"><span aria-hidden="true">←</span> 편집 종료</button></header><form onSubmit={submit}>
    ...
    <label className="profile-edit-field"><span>커버 위 한마디</span><div><input value={values.headline} onChange={event => update('headline', event.target.value)} maxLength="40" placeholder="팬들에게 전하고 싶은 한마디" /><small className="profile-headline-count">{values.headline.length} / 40자</small></div></label>
    <label className="profile-edit-field"><span>소개글</span><div><textarea value={values.bio} onChange={event => update('bio', event.target.value)} maxLength="500" placeholder="나를 소개하는 글을 작성해 주세요." /></div></label>
    ...
    {saveStatus && <p className="profile-edit-status" role="status">{saveStatus}</p>}<footer><button type="button" onClick={onClose}>취소</button><button type="submit" disabled={saving}>{saving ? '저장 중…' : '프로필 저장'}</button></footer>
...
function FanStatsGuide({ type = 'credit', onClose }) {
  ...
  const guides = isHeat ? [
    ...
    ['활동 반영', '포스트 작성과 댓글 등 FANHEAT 안에서 참여한 활동이 HEAT RANGE에 반영됩니다.'],
    ['MAX', 'HEAT RANGE를 100%까지 달성한 누적 횟수입니다.'],
    ['TTL', '현재 HEAT가 유지되는 남은 기간입니다. 기간이 끝나기 전에 활동을 이어가 보세요.'],
  ] : [
    ['FAN CREDIT', 'FANHEAT 활동으로 쌓이는 성장 포인트입니다. 누적 FC에 따라 레벨이 올라갑니다.'],
    ['LEVEL', '누적된 FAN CREDIT에 따라 결정되는 나의 팬 활동 레벨입니다.'],
    ['RANK', '전체 팬 가운데 현재 활동과 FAN CREDIT을 기준으로 계산한 나의 순위입니다.'],
  ]
...
function MyPageProfile({ user, profile, tracks = [], onBack }) {
  ...
  const [fanCred, setFanCred] = useState(0)
  ...
  useEffect(() => {
    const panel = document.querySelector('.my-page-profile')
    const carousel = panel?.querySelector('.my-avatar-carousel')
    const image = carousel?.querySelector(':scope > img')
    if (!carousel || !image) return undefined
    let pointerId = null
    let startX = 0
    let dragX = 0
    const resetDrag = () => {
    ...
    const moveDrag = event => {
      ...
      dragX = Math.max(-90, Math.min(90, event.clientX - startX))
      image.style.transform = `translateX(${dragX}px)`
    }
    const finishDrag = event => {
      if (event.pointerId !== pointerId) return
      carousel.releasePointerCapture?.(event.pointerId)
      if (Math.abs(dragX) >= 36) setSlide(current => (current + (dragX < 0 ? 1 : -1) + profileSlides.length) % profileSlides.length)
      resetDrag()
    }
  ...
  useEffect(() => {
    let active = true
    if (profile || !user?.id) return undefined
    loadFanCred(user.id).then(score => { if (active) setFanCred(score) }).catch(() => {})
    return () => { active = false }
  }, [profile, user?.id])
  ...
  const fanStatSource = profile || user?.user_metadata || {}
  const storedHeat = Math.max(0, Math.min(100, Number(fanStatSource.activity ?? fanStatSource.heat_range ?? 80)))
  const heatMaxCount = Math.max(0, Number(fanStatSource.heatMaxCount ?? fanStatSource.heat_max_count ?? 12))
  const heatSnapshot = calculateHeatSnapshot({ heatRange: storedHeat, lastActivityAt: fanStatSource.lastActivityAt || fanStatSource.last_heat_activity_at || Date.now() })
  const heat = heatSnapshot.heatRange
  const heatDaysRemaining = heatSnapshot.daysRemaining
  const fanLevelStats = calculateFanLevel(fanCred)
  const fanLevel = fanLevelStats.level
  const credProgress = fanLevelStats.progress
  const fanRank = calculateFanRank(fanStatSource.rank ?? fanStatSource.fan_rank ?? 96, fanStatSource.rankTotal ?? fanStatSource.fan_rank_total ?? 2400)
...
function MessageCenter({ user, onUnreadChange }) {
  ...
  return <section className="message-center">
    ...
    {folder === 'blocked' ? <div className="message-block-list">{blocks.map(block => <article key={block.blocked_user_id}><img src={assetSrc(block.blocked?.avatar_url || 'mypage.jpg')} alt="" /><div><strong>{block.blocked?.display_name || 'FAN'}</strong><span>{formatMessageTime(block.created_at)}부터 차단됨</span></div><button type="button" disabled={busy} onClick={() => toggleBlock(block.blocked, false)}>차단 해제</button></article>)}{!blocks.length && <p>차단한 사용자가 없습니다.</p>}</div> : <div className="message-list-shell">
      <label className="message-search"><span aria-hidden="true">⌕</span><input value={query} onChange={event => setQuery(event.target.value)} placeholder="이름 또는 내용 검색" aria-label="쪽지 검색" /></label>
...
function MyPageContent({ user, posts: items, followers, unreadMessageCount = 0, onSelect, onOpenFriend, onUnreadChange, initialTab = 'posts', publicProfile = null }) {
  ...
  const demoFollowers = [
    ['@chaewon_fh', 0, 'mypage.jpg'],
    ['@hyunwoo_star', 0, 'chart1.jpg'],
    ['@minseo_luv', 0, 'post2.jpg'],
    ['@harin_day', 0, 'chart7_2.jpg'],
    ['@jiwoo_wave', 0, 'ive-jacket.jpeg'],
    ['@seojun_02', 0, 'chart11.jpg'],
    ['@yuna_archive', 0, 'rescene-jacket.jpeg'],
    ['@doyoon_fan', 0, 'chart3.jpg'],
    ['@sua_cloud', 0, 'bingle_bangle.jpg'],
    ['@yejun_heat', 0, 'chart9.jpg'],
    ['@naeun_note', 0, 'mypage_bg.jpg'],
    ['@siwoo_music', 0, 'chart6.jpg'],
  ]
  followers = isPublicProfile ? [] : demoFollowers
  const [friends, setFriends] = useState(() => followers.map((friend, index) => [...friend, 100 - index * 6, {
    starLevel: Math.max(0, 5 - index),
    sharedPostCount: Math.max(0, 8 - index),
    commentsOnMyPosts: Math.max(0, 6 - index),
    repliesToMyComments: Math.max(0, 4 - index),
    demo: true,
  }]))
  const [friendQuery, setFriendQuery] = useState('')
  const [friendInvites, setFriendInvites] = useState([['@karina_wave', 0, 'chart8.jpg', 44], ['@winter_note', 0, 'chart10.jpg', 38]])
...
function Awards({ items = awards, onSelect, onViewAll }) {
  ...
  const periodProfiles = {
    Today: { current: [1, 0, 4, 2, 7, 3, 5, 6], previous: [4, 2, 0, 6, 3, 5, 1], newcomers: [7], factor: .18 },
    Weeks: { current: [0, 1, 2, 3, 4, 5, 6, 7], previous: [2, 0, 6, 4, 7, 3, 1, 5], newcomers: [], factor: 1 },
    Month: { current: [2, 0, 1, 6, 3, 4, 5, 7], previous: [7, 3, 5, 1, 4, 0, 6, 2], newcomers: [], factor: 4.2 },
  }
  const profile = periodProfiles[period]
  const arrange = order => {
    const included = new Set(order)
    return [...order.map(index => ({ item: items[index], originalIndex: index })).filter(entry => entry.item), ...items.map((item, originalIndex) => ({ item, originalIndex })).filter(entry => !included.has(entry.originalIndex))]
  }
  const currentSnapshot = arrange(profile.current)
  const previousSnapshot = arrange(profile.previous).filter(entry => !profile.newcomers.includes(entry.originalIndex))
  const previousRanks = new Map(previousSnapshot.map((entry, index) => [entry.originalIndex, index + 1]))
  const highestBaseScore = Math.max(1, ...items.map(([, score]) => Number(String(score).replace(/,/g, '')) || 0))
  const rankedItems = currentSnapshot.map((entry, index) => {
    const rank = index + 1
    const previousRank = previousRanks.get(entry.originalIndex)
    const change = previousRank == null ? null : previousRank - rank
    const score = Math.round(highestBaseScore * profile.factor * Math.pow(.87, index) + (entry.originalIndex + 1) * 17)
    return { ...entry, rank, change, score: score.toLocaleString('ko-KR') }
  })
  const measure = () => {
  ...
  return <section className="awards" id="awards">
    ...
    <div className="award-carousel" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      ...
      <div className="award-grid" ref={track}>{rankedItems.map(({ item, originalIndex, rank, change, score }) => { const [name, , image] = item; const isNew = change == null; const trendClass = isNew ? 'new' : change > 0 ? 'up' : change < 0 ? 'down' : 'same'; const trendText = isNew ? 'NEW' : change > 0 ? `▲ ${change}` : change < 0 ? `▼ ${Math.abs(change)}` : '—'; const trendLabel = isNew ? '이번 기간 신규 진입' : change > 0 ? `직전 기간보다 ${change}계단 상승` : change < 0 ? `직전 기간보다 ${Math.abs(change)}계단 하락` : '직전 기간과 동일한 순위'; return <article key={`${period}-${name}-${originalIndex}`}>
        <button className="award-card" onClick={() => onSelect?.(makeStarProfile(item))} aria-label={`${name}, ${rank}위, ${trendLabel}`}>
          <span className="award-image"><img src={assetSrc(image)} alt={name} /></span>
          <span className="award-caption"><strong><i>{rank}위</i> {name}</strong><small>{score}</small><em className={`rank-change ${trendClass}`} title={trendLabel}>{trendText}</em></span>
        </button>
      </article> })}</div>
...
function ArtistDirectory({ items, query, setQuery, onSelect }) {
  ...
  return <section className="artist-directory">
    ...
    <div className="artist-directory-grid">
      {visible.map((item, index) => { const [name, score, image, data] = item; const originalIndex = items.indexOf(item); const hasMaster = Boolean(data?.master_user_id || data?.master_id || data?.has_master || [0, 3, 6, 9].includes(originalIndex)); const topTracks = artistTopTracks[name] || artistTopTracks[data?.name] || ['대표 싱글', '인기 타이틀', '팬 추천곡', '라이브 인기곡', '최신 발매곡']; return <button className={`artist-directory-card artist-palette-${originalIndex % 8}`} key={data?.id || `${name}-${index}`} onClick={() => onSelect(makeStarProfile(item))}>
...
function AlbumDetailModal({ star, album, onClose }) {
  ...
  const featuredTrack = tracks.find(track => track.youtube_url) || tracks[0]
  const videoUrl = featuredTrack?.youtube_url || album.youtubeUrl
      ...
      <div className="album-modal-grid">
        <div className="album-modal-left">
          ...
          <div className="track-table" role="table" aria-label={`${album.title} 수록곡`}>
            ...
            {tracks.map(track => <div className="track-row" role="row" key={track.id}><span>{track.track_number}</span><strong>{track.title}{track.title === album.track && <em>TITLE</em>}</strong><span>{track.duration_text || '—'}</span>{track.youtube_url ? <a href={track.youtube_url} target="_blank" rel="noreferrer" aria-label={`${track.title} YouTube에서 보기`}>▶</a> : <span>—</span>}</div>)}
        ...
        <div className="album-modal-right">
          ...
          <div className="album-video-frame">{embedUrl ? <iframe src={embedUrl} title={`${star.name} ${featuredTrack?.title || album.track} 영상`} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen /> : <div className="album-db-empty">등록된 YouTube 영상이 없습니다.</div>}</div>
          <div className="album-lyrics-title"><h3>곡 소개</h3><span>TRACK NOTE</span></div>
...
function StarAlbums({ star }) {
  ...
  const catalog = (star.albums || []).map(album => ({ id: album.id, title: album.title, track: album.lead_track || album.title, date: album.release_date?.replaceAll('-', '.') || '', type: album.album_type || '앨범', tracks: Number(album.track_count || 0), image: album.cover_url || star.image, youtubeUrl: album.youtube_url || '', description: album.description || '', label: album.label || '', genre: album.genre || '', externalUrl: album.external_url || '', trackItems: album.artist_album_tracks || [] }))
  ...
  return <section className="star-album-section">
    ...
    {!catalog.length && <p className="star-gallery-empty">Supabase에 등록된 앨범이 없습니다.</p>}
...
function StarGallery({ star }) {
  ...
  const galleryItems = (star.galleryItems || []).map(item => ({ image: item.image_url, title: item.title, date: item.captured_on?.replaceAll('-', '.') || '', timestamp: item.captured_on ? new Date(`${item.captured_on}T00:00:00`).getTime() : Date.now() }))
  ...
  const lightboxImages = visibleItems.map((item, index) => ({ type: 'image', src: item.image, imageIndex: index }))
    ...
    <div className="star-gallery-grid">
      {visibleItems.map(({ image, title, date }, index) => {
        return <article className="star-gallery-card" key={`${image}-${index}`}>
          <button className="star-gallery-image" type="button" onClick={() => setLightboxIndex(index)} aria-label={`${title} 이미지 크게 보기`}><img src={assetSrc(image)} alt={`${star.name} ${title}`} loading="lazy" /><span aria-hidden="true">크게 보기</span></button>
          <h3>{title}</h3>
...
function StarPage({ star, onOpenFan }) {
  ...
  return <section className="star-page">
    <nav className="star-tabs" aria-label="스타 정보 탭">{tabs.map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)} aria-selected={tab === key}>{label}{key === 'albums' && <b>{star.albums?.length || 0}</b>}</button>)}</nav>
...
function Feed({ query, filters, onSelect, items = posts, user, onLogin, onHeatChange }) {
  ...
  return <section className="feed" id="feed">
    <div className="post-list post-list-mobile-card">{visiblePosts.map(({ item: [title, image, data], originalIndex }, index) => {
      ...
      <header className="post-mobile-header"><span className="post-mobile-avatar" aria-hidden="true">{authorName.slice(0, 1).toUpperCase()}</span><div><strong>{authorName}</strong><small>@{authorId} · {relativePostTime(data?.published_at || data?.created_at)}</small></div><button type="button" onClick={event => event.stopPropagation()} aria-label="게시물 메뉴">•••</button></header>
...
function PostDetail({ post, onClose, user, onLogin, onEdit, previousPost, nextPost, onNavigate, onHeatChange, onOpenAuthor }) {
  ...
  const friendRequestKey = `fanheat-friend-request-${post.data?.author_id || 'unknown'}`
  const [friendRequested, setFriendRequested] = useState(() => window.localStorage.getItem(friendRequestKey) === 'sent')
  const dragStart = useRef(null)
  ...
  const detailSlides = [
    ...
    ...detailImages.map((src, imageIndex) => ({ type: 'image', src, imageIndex })),
  ...
  useEffect(() => { setSlide(0); setLightboxIndex(null) }, [post.data?.id])
  useEffect(() => {
  ...
  useEffect(() => setFriendRequested(window.localStorage.getItem(friendRequestKey) === 'sent'), [friendRequestKey])
  ...
  const requestFriend = () => {
    if (!user) { onLogin?.(); return }
    window.localStorage.setItem(friendRequestKey, 'sent')
  ...
  return <section className={`post-detail ${isScrolling ? 'is-scrolling' : ''}`} onScroll={revealScrollbar}>
    ...
    <div className="detail-body">
      ...
      <div className="detail-credits">
        ...
        <div className="author"><span>작성자</span><button className="author-profile-link" type="button" onClick={() => onOpenAuthor?.({ userId: post.data?.author_id, id: `@${authorName}`, displayName: authorName, image: 'mypage.jpg', artist: 'FANHEAT' })}>@{authorName}</button>{!isOwner && <button className={friendRequested ? 'requested' : ''} type="button" onClick={requestFriend} disabled={friendRequested}>{friendRequested ? '요청 보냄' : '친구 추가'}</button>}</div>
      </div>
...
function WriteEditor({ draft, setDraft, images, setImages, imageFiles, setImageFiles, inlineImages, setInlineImages, inlineImageFiles, setInlineImageFiles, onClose, onPublish, editing = false }) {
  ...
  const addInlineImages = (files, previews) => {
    setInlineImages(current => [...current, ...previews])
  ...
  const addRepresentativeImages = event => {
    ...
    setImages(current => [...current, ...files.map(file => URL.createObjectURL(file))])
  ...
  const removeRepresentativeMedia = () => {
    ...
    setImages([]); setImageFiles([]); setDraft(current => ({ ...current, mediaUrl: '', mediaItems: [] }))
  }
  ...
  return <section className="write-editor" aria-label="게시글 작성">
    ...
    <div className="write-field required editor-content-field"><span>{t('content')}</span><div><p className="editor-media-guide">여기는 게시글 <b>본문</b>입니다. 위의 대표 미디어와 별개로 글과 본문 이미지를 편집할 수 있습니다.</p><RichTextEditor value={draft.content} onChange={value => update('content', value)} onAddImages={addInlineImages} remainingImages={Math.max(0, 5 - inlineImages.length)} /></div></div>
...
function FanHeatApp() {
  ...
  useEffect(() => {
    ...
    const baseTitle = 'FANHEAT | K-POP 아티스트·팬 커뮤니티·K-컬처'
    const baseDescription = 'FANHEAT에서 K-POP 아티스트, 최신 앨범과 컴백, 콘서트 일정, 팬 콘텐츠와 K-컬처 소식을 한곳에서 만나보세요.'
  ...
  const navigatePost = post => {
    ...
    window.requestAnimationFrame(() => document.querySelector('.post-detail')?.scrollTo({ top: 0, behavior: 'smooth' }))
  }
